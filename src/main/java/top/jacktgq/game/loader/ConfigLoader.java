package top.jacktgq.game.loader;

import top.jacktgq.game.collider.Collider;
import top.jacktgq.game.factory.abstractfactory.GameFactory;
import top.jacktgq.game.strategy.FireStrategy;
...
public class ConfigLoader {
    ...
    public static FireStrategy getEnemy_tank_fs() {
        try {
            Class<?> clazz = Class.forName("top.jacktgq.game.strategy." + get("enemy_tank_fs"));
    ...
    public static FireStrategy getSelf_tank_fs() {
        try {
            Class<?> clazz = Class.forName("top.jacktgq.game.strategy." + get("self_tank_fs"));
            return (FireStrategy) clazz.newInstance();
        } catch (Exception e) {
            e.printStackTrace();
            throw new RuntimeException("开火策略读取失败！");
    ...
    public static GameFactory getFactory() {
        try {
            Class<?> clazz = Class.forName("top.jacktgq.game.factory." + get("factory_style"));
    ...
    public static LinkedList<Collider> getColliders() {
        try {
            ...
            for (String colliderName : colliderNames) {
                Class<?> clazz = Class.forName("top.jacktgq.game.collider." + colliderName);
